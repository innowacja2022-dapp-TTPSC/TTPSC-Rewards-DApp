...
export const RemoveAward = ({ award }: Props): ReactElement => {
  const rewardManagerService = useRewardManagerService();

  const [name, setName] = useState(award.name);
  const [amount, setAmount] = useState(award.amount);
  const [price, setPrice] = useState(award.price);
  const [image, setImage] = useState<Buffer | undefined>(award.image);
